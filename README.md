# golang-demo-app - Argo CD Application (QA)

This repository contains the Argo CD Application manifest for the **QA** environment of the golang-demo-app essesseff™ app.  
...
```
golang-demo-app-argocd-qa/
├── app-of-apps.yaml.template                      # Root Application template
├── argocd/
│   └── golang-demo-app-qa-application.yaml.template   # QA environment Application manifest (auto-synced) template
...
│   └── offboard-golang-demo-app-qa.sh    # script for offboarding the golang-demo-app qa app 1) from essesseff only or 2) from Argo CD and K8s entirely
├── setup-argocd-cluster.sh           # Argo CD K8s setup script 
├── setup-argocd.sh                   # Argo CD golang-demo-app-qa essesseff app setup script 
└── README.md                         # This file
```
...
### Configure golang-demo-app-qa essesseff App to Argo CD and deploy to K8s

***Note: it is highly recommended that, rather than going through all the steps here to configure Argo CD, that for convenience and faster turnaround time you instead use the [essesseff-onboarding-utility](https://github.com/essesseff/essesseff-onboarding-utility)***

1. **Git Clone This Repository**:
   ```bash
   git clone git@github.com:essesseff-demo-account/golang-demo-app-argocd-qa.git
   ```
...
2. **Configure Environment Variables in .env File**:
   ...
   Then set the environment variables in the .env which will be used for generating the following from the templates in this repository:
      ...
      - `golang-demo-app-argocd-qa` repository (to read Application manifests)
      - `golang-demo-app-config-qa` repository (to read Helm charts and values)
      ...
      c. ***Configuration of golang-demo-app-qa Deployment in Argo CD***:

      golang-demo-app-qa-application.yaml is used to configure the golang-demo-app-qa deployment

      d. ***Configuration of golang-demo-app-argocd-qa App-of-Apps Deployment in Argo CD***:

      app-of-apps.yaml is used to configure the golang-demo-app-argocd-qa app-of-apps deployment

      e. ***Configuration of golang-demo-app-qa Argo CD Notifications to essesseff***:

      If notifications-secret.yaml is downloaded from essesseff for golang-demo-app-qa, notifications-configmap.yaml will be used to configure Argo CD notifications to essesseff.
...
4. **Run the setup-argocd.sh Script**:
   ...
   This script applies all secrets, configmaps, Argo CD application definitions, etc. for golang-demo-app QA.
...
5. **Verify in Argo CD UI**:
   ...
   You should see:
   - `golang-demo-app-argocd-qa` - Root Application (watches this repository)
   - `golang-demo-app-qa` - Environment Application (auto-synced by root Application)

6. **Access the Deployed Application**:
   ```bash
   kubectl port-forward service/golang-demo-app-qa 8081:80 -n essesseff-demo-account
   # Access: http://localhost:8081
   ```
### How to Offboard golang-demo-app-qa Deployment from Argo CD and K8s

1. **Execute the offboarding script**:
   ```bash
   cd offboarding
   chmod 744 offboard-golang-demo-app-qa.sh
   ./offboard-golang-demo-app-qa.sh
   ```
...
- **Name**: `golang-demo-app-qa`
- **Namespace**: `argocd`
- **Source Repository**: `golang-demo-app-config-qa`
...
3. essesseff GitHub App automation triggers essesseff to auto-update Helm `golang-demo-app-config-qa/Chart.yaml` and `golang-demo-app-config-qa/values.yaml` with the image tag of the newly built image
4. Argo CD syncs QA Application automatically on K8s

### Manual GitOps Deployment

1. Push update(s) to the `main` branch in `golang-demo-app-config-qa` repository, typically to Helm Chart.yaml and/or values.yaml
2. Argo CD syncs QA Application automatically on K8s

## Repository URLs

- **Source**: `https://github.com/essesseff-demo-account/golang-demo-app`
- **Config QA**: `https://github.com/essesseff-demo-account/golang-demo-app-config-qa`
- **Argo CD QA**: `https://github.com/essesseff-demo-account/golang-demo-app-argocd-qa` (this repo)
...
- Allow Argo CD to detect changes in `argocd/golang-demo-app-qa-application.yaml` more quickly
